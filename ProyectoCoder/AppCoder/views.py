...
""" --ABM Usuario--
def eliminarUsuario(request, numero_para_borrar):
    
    UsuarioQueQuieroBorrar = Usuario.objects.get(numero=numero_para_borrar)
    UsuarioQueQuieroBorrar.delete()
    
    
    Usuarios = Usuario.objects.all()
    
    return render(request, "AppCoder/leerUsuarios.html", {"Usuarios":Usuarios} )


@login_required
def leerUsuarios(request):
    
    Usuarios = Jugador.objects.all()
    
    dir = {"Usuarios":Usuarios} #contexto
    
    return render(request, "AppCoder/leerUsuarios.html", dir)
...
def eliminarPosteo(request, numero_para_borrar):
    ...
    return render(request, "AppCoder/leerPosteos.html", {"Posteos":Posteos} )


@login_required
def leerPosteos(request):
    
    Posteos = Jugador.objects.all()
    
    dir = {"Posteos":Posteos} #contexto
    
    return render(request, "AppCoder/leerPosteos.html", dir)
...
def buscarPosteo(request):
    ...
    if request.GET["nombre"]:
        ...
        posteos = Posteo.objects.filter(nombre__icontains=nombre)
        
        
        #respuesta = f"ESTOY BUSCANDO A: {request.GET['nombre']}"
        
        return render(request, "AppCoder/resultadoBusqueda.html",{"posteos":posteos, "nombre":nombre})
...
def empleadoFormulario(request):
    ...
    if request.method == "POST":
        ...
        if miFormulario.is_valid():  
            ...
            usua.save() #Es el que guarda en la BD
...
def posteoFormulario(request):
    ...
    if request.method == "POST":
        ...
        if miFormulario.is_valid():  
            ...
            poste.save() #Es el que guarda en la BD
    ...
    #return HttpResponse("Esto es una prueba del inicio")
    return render(request, 'AppCoder/posteeoFormulario.html',{"miFormulario":miFormulario})
"""
